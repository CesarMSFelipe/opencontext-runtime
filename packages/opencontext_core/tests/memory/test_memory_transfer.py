"""Tests for opencontext_core.memory.transfer — export/import helpers.

S3 RED: these must fail until transfer.py is created and main.py delegates to it.
"""
...
from opencontext_core.memory.transfer import memory_export, memory_import
from opencontext_core.memory_usability.context_repository import ContextRepository


def test_export_import_round_trip_via_core(tmp_path: Path) -> None:
    """memory_export/memory_import from core module round-trip correctly."""
...
def test_import_missing_file_exits_via_core(tmp_path: Path) -> None:
    dst = ContextRepository(tmp_path / "b")
    try:
        memory_import(dst, str(tmp_path / "nope.json"))
